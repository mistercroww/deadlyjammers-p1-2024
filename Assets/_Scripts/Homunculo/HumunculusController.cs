...
public class HumunculusController : MonoBehaviour
{
    ...
    private void IncreaseHunger()
    {
        ...
        if (_hungerDecreaseRateCounter < 0)
        {
            if ((hungerIndicator - hungerDecressAmount) < 100)
            {
                ...
                if (Random.Range(0, 11) == 5 && hungryEvent != null &&
                    ExtensionMethods.Percentage(hungerIndicator, 100) >= 50f)
                {
                    hungryEvent.Invoke();
                    if (playerSanityAffect != null)
                    {
    ...
    private void IncreaseFun()
    {
        ...
        {
            if ((funIndicator - funDecressAmount) < 100 && _radioController.isOn)
            {
                funIndicator = ExtensionMethods.AddToValueWithMax(funIndicator, funDecressAmount, 100);
            }
            else
            {
                funIndicator = ExtensionMethods.SubtractToValueWithMin(funIndicator, funDecressAmount, 0);
            }
